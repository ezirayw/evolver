...
import aiohttp
import requests
from evolver.robotics_server import OctoPrintInterface
import socketio
import asyncio
import shutil
import logging
import time
from dataclasses import dataclass, field, asdict, cast
...
class GeneralPumpConfig(TypedDict):
    priming_steps: int
    plunger_speed_int: int
    plunger_speed_out: int
    max_speed: int
    pump_positions: int
...
class PumpConfig(TypedDict):
    ...
    max_steps: int
    valve_steps: int
    control_interface: OctoPrintInterface
    valve_motor: X # connection stepper motor that controls valve is connected to (either X, Y, Z, E0, E1)
    plunger_motor: Y #

# dataclass for pump
class RoboticsPump:
    def __init__(self, general_pump_conf: GeneralPumpConfig, specific_pump_conf: ) -> None:
        pass
...
class OctoPrintInterface:
    ...
    def cancel(self):
        ...
        try:
            ...
            raise OctoPrintError(
                "could not cancel pump jobs on %s due to error: %s" % (self.name, e)
            )
    ...
    def write_gcode(self, mode, pump_instructions, pump_conf, primed_status, gcode=""):
        ...
        if mode == "aspirate":
            for pump in pump_instructions:
                ...
                plunger_in_commands[count] = "{0}{1}".format(
                    plunger, pump_instructions[pump]
                )
                count = count + 1
            ...
            gcode = "G91\nG1 {0} F{1}\nM18".format(
                command, pump_conf["plunger_speed_in"]
            )
            ...
            for pump in pump_instructions:
                ...
                plunger_out_commands[count] = "{0}-{1}".format(
                    plunger, pump_instructions[pump]
                )
                if primed_status and pump_instructions[pump] != 0:
                    plunger_out_commands[count] = "{0}-{1}".format(
                        plunger, pump_instructions[pump] + pump_conf["priming_steps"]
                    )
                    prime_commands[count] = "{0}{1}".format(
                        plunger, pump_conf["priming_steps"]
                    )
            ...
            gcode = "G91\nG1 {0} F20000\nG4 P100\nG1 {1} F{2}\nG4 P100\nG1 {3} F20000\nM18".format(
                valve_on, plunger_out, pump_conf["plunger_speed_out"], valve_off
            )
            if primed_status and pump_instructions[pump] != 0:
                gcode = "G91\nG1 {0} F20000\nG4 P100\nG1 {1} F{2}\nG4 P100\nG1 {3} F15000\nG4 P100\nG1 {4} F20000\nM18".format(
                    valve_on,
            ...
            valve_off = valve_commands["off"][0] + " " + valve_commands["off"][1]
            gcode = "G91\nG1 {0} F20000\nG4 P100\nG1 {1} F{2}\nG4 P100\nG1 {3} F20000\nM18".format(
                valve_on, plunger_in_commands, pump_conf["plunger_speed_in"], valve_off
            )
...
class RoboticsServer(EvolverNamespace):
    def __init__(self):
        ...
        self.robotics_status = {
            ...
            "pumps": {},
            "prime_status": {"influx": False, "efflux": False},
        }
        ...
        for octoprint_name in self.robotics_conf["octoprint_instances"]:
            self.octoprint_instances[octoprint_name] = OctoPrintInterface(
                ...
                octoprint_name,
                self.robotics_conf["post_request_timeout"],
            )
            self.robotics_status["OctoPrint"].update(
                {
                    octoprint_name: {
                        "connection_status": self.octoprint_instances[
                            octoprint_name
                        ].connected
                    }
                }
        ...
        for octoprint_name in self.octoprint_instances:
            ...
            for pump_target in instructions:
                if (
                    ...
                    and self.pump_conf["pumps"][pump_target]["octoprint_id"]
                    == self.octoprint_instances[octoprint_name].id
                ):
    ...
    async def fluidic_event(
        self, session, gcode_files, mode, arm_settings={}, print_string=""
    ):
        ...
        )  # hang here if pause is called before starting aspiration commands
        ...
        async with asyncio.TaskGroup() as aspiration_tasks:
            for octoprint_name in self.octoprint_instances:
                gcode_path = os.path.join(
                    self.octoprint_instances[octoprint_name].gcode_folder,
                    gcode_files[0],
                )
    ...
    async def prime_efflux_helper(self, data):
        ...
        if (
            self.robotics_status["mode"] == "idle"
            and not self.robotics_status["prime_status"]["efflux"]
        ):
            # update robotics status
            ...
            for quad in data["active_quads"]:
                for ipp_address in ipp_addresses[quad]:
                    ipp_effux_command[ipp_address] = "{0}|{1}|{2}|{3}".format(
                        self.pump_conf["efflux_priming_settings"]["frequency"],
                        ipp_number,
                        ipp_index,
                        self.pump_conf["efflux_priming_settings"]["ipp_time"],
                    )
    ...
    async def fill_tubing_helper(self):
        ...
        if self.robotics_status["mode"] == "idle":
            ...
            mapped_fill_tubing_instructions = self.map_gcode_commands(
                fill_tubing_instructions
            )
            ...
            for octoprint_name in mapped_fill_tubing_instructions:
                self.octoprint_instances[octoprint_name].write_gcode(
                    "aspirate",
                    mapped_fill_tubing_instructions[octoprint_name],
                    self.pump_conf,
                    self.robotics_status["prime_status"]["influx"],
                )
                self.octoprint_instances[octoprint_name].write_gcode(
            ...
            try:
                await self.fluidic_event(
                    session, gcode_files, "fill_tubing", print_string=print_string
                )
                await session.close()
                self.robotics_status["mode"] = "idle"
                ...
                self.stop_robotics()
                raise HelperEventError(
                    "error running fill_tubing_helper, check logs for traceback"
                )
        ...
        if (
            ...
            for quad_name in data["active_quads"]:
                ...
                z_wash_station = np.array(
                    [
                        self.robotics_conf["homing_coordinates"][quad_name][
                            "wash_station"
                        ]["z_out"],
                        self.robotics_conf["homing_coordinates"][quad_name][
                            "wash_station"
                ...
                transform_matrices = np.stack(
                    (transform_matrix_out, transform_matrix_in)
                )
                    ...
                    for x in range(num_vial_windows):
                        if not uniform_pump_types:
                            vial_window = current_vial_row[
                                num_vial_windows * x : num_vial_windows * x
                                + num_vial_windows
                                + 1
                            ]
                        ...
                        for i in range(len(vial_window)):
                            print_string = print_string + "vial_{0} ".format(
                                vial_window[i]
                            )
                        ...
                        if data["wash"]:
                            ...
                            try:
                                ...
                                self.stop_robotics()
                                raise HelperEventError(
                                    "error running wash fluidic event in influx_snake_helper, check logs for traceback"
                                )
                        ...
                        while True:
                            ...
                            for pump in active_pumps:
                                if max_pump_num[pump] > 0:
                                    pump_instructions[pump] = self.pump_conf["pumps"][
                                        pump
                                    ]["max_steps"]
                                    max_pump_num[pump] = max_pump_num[pump] - 1

                                if max_pump_num[pump] == 0:
                                    pump_instructions[pump] = (
                                        fractional_pump_instructions[pump]
                                    )
                            ...
                            if not arm_moved:
                                ...
                                z = {
                                    "current": z_wash_station,
                                    "target": z_vial_dilution,
                                }
                                arm_settings = {
                            ...
                            try:
                                ...
                                self.stop_robotics()
                                raise HelperEventError(
                                    "error running vial_set fluidic event in influx_snake_helper, check logs for traceback"
                                )
                ...
                await (
                    self.check_for_pause()
                )  # hang here if pause is called before moving arm to next row
                try:
                    ...
                    self.stop_robotics()
                    raise HelperEventError(
                        "error moving arm up after finishing snake influx path for %s, check logs"
                        % quad_name
                    )
            ...
            return {
                "done": False,
                "message": "robotics_status mode is not idle or pause",
            }
    ...
    async def move_arm(self, coordinates, wait):
        ...
        if (
            self.robotics_status["mode"] == "emergency_stop"
            or self.robotics_status["xArm"]["arm_state"] == 4
        ):
            ...
            if result < 0:
                raise RoboticsError(
                    "failure moving xARM during move_arm(), error code {0} given".format(
                        result
                    )
                )
            return result
    ...
    async def on_fill_tubing_routine(self, sid, data):
        ...
        try:
            ...
            return {
                "done": result["done"],
                "routine": "fill_tubing",
                "robotics_status": self.robotics_status,
    ...
    def setup_event_handlers(self):
        ...
        self.sio.on(
            "fill_tubing_routine", self.on_fill_tubing_routine, namespace="/robotics"
        )
        self.sio.on(
            "prime_influx_routine", self.on_prime_influx_routine, namespace="/robotics"
        )
        self.sio.on(
            "prime_efflux_routine", self.on_prime_efflux_routine, namespace="/robotics"
        )
        ...
        self.sio.on(
            "request_pump_conf", self.on_request_pump_conf, namespace="/robotics"
        )
        self.sio.on(
        ...
        self.sio.on(
            "reconnect_robotics", self.on_reconnect_robotics, namespace="/robotics"
        )
